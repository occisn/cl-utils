...
(defmacro with-collector--reversed-order ((collect) &body body)
    "Return the list of x generated by (collect x) within BODY.
Order is reversed (quicker).
(v1 available in occisn/cl-utils GitHub repository)"
  (let ((lst (gensym "LST")))
    `(let ((,lst nil))
       (flet ((,collect (x) (push x ,lst)))
         ,@body)
       ,lst)))
...
(defmacro with-collector ((collect) &body body)
    "Return the list of x generated by (collect x) within BODY.
...
(defun SHOW-all-macros ()
  ...
  (SHOW-ablock)
  (format t "~%")
  (SHOW-with-collector--reversed-order)
  (format t "~%")
  (SHOW-with-collector)
  (format t "~%")
  (SHOW-let+))
